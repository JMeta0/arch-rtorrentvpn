...
github.sh --install-path '/tmp/libtorrent/src' --github-owner 'rakshasa' --github-repo 'rtorrent' --download-assets "libtorrent.*tar.gz" --query-type 'release' --compile-src "cd /tmp/libtorrent/src/libtorrent* && sed '/AM_PATH_CPPUNIT/d' -i configure.ac && autoreconf -f -i && export CXXFLAGS=\"${CXXFLAGS} -fno-strict-aliasing\" && ./configure --prefix=/usr --disable-debug && make && make DESTDIR='/' install"
